...
class WorkflowRequest(BaseModel):
    ...
    prompt: Optional[str] = None  # Natural language trigger for the workflow
...
async def get_catalog_sources():
    ...
    try:
        # Query to get all tables/views with row counts and metadata
        query = """
        SELECT 
        ...
        sources = []
        for row in result:
            # Determine status based on accessibility
            status = 'ready'  # Default to ready if we can see it
            ...
            sources.append({
                ...
                'hasSemanticModel': False  # Will check separately
            })
        
        return {"sources": sources}
    except Exception as e:
        print(f"Catalog error: {e}")
        # Return empty list instead of error - frontend has fallback
...
async def get_tools():
    ...
    try:
        ...
        # Handle case where result is not a list of dicts
        if not result or not isinstance(result, list):
            return {"tools": []}
        
        tools = []
        for row in result:
            # Skip if row is not a dict (e.g., error message)
        ...
        # Return empty list - don't block the UI
        return {"tools": []}
...
async def get_templates():
    ...
    try:
        ...
        # Return empty list - frontend has hardcoded fallback templates
        return {"templates": []}
...
def log_audit(action_type: str, entity_type: str, entity_id: str, entity_name: str, details: dict = None):
    ...
    try:
        ...
        print(f"Audit log error: {e}")  # Don't fail the main operation
