# Retail Grocer: Power BI ↔ Snowflake Multi-Domain Flow

## Visual Architecture

```
┌──────────────────────────────────────────┐         ┌──────────────────────────────────────────────────────────────────┐
│           🟦 MICROSOFT SIDE              │         │                    ❄️ SNOWFLAKE SIDE                             │
│          (Power BI / Fabric)             │         │                   (Cortex / Warehouse)                          │
├──────────────────────────────────────────┤         ├──────────────────────────────────────────────────────────────────┤
│                                          │         │                                                                  │
│  ┌────────────────────────────────────┐  │         │                                                                  │
│  │  👤 User asks in Power BI Copilot  │  │         │                                                                  │
│  │  "Why did margin drop in Scotland?"│  │         │                                                                  │
│  └─────────────────┬──────────────────┘  │         │                                                                  │
│                    │                     │         │                                                                  │
│                    ▼                     │         │                                                                  │
│  ┌────────────────────────────────────┐  │         │                                                                  │
│  │      📊 TMDL Semantic Models       │  │         │                                                                  │
│  │      (DAX measures, tables)        │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  📈 Sales_Revenue.tmdl             │  │         │                                                                  │
│  │     • TotalSales, LFLSales         │  │         │                                                                  │
│  │     • GrossMarginPct, ATV          │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  📦 Inventory_Supply.tmdl          │  │         │                                                                  │
│  │     • StockTurn, InStockRate       │  │         │                                                                  │
│  │     • DaysOfSupply, OTIF           │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  👥 Customer_Loyalty.tmdl          │  │         │                                                                  │
│  │     • CLV, RetentionRate           │  │         │                                                                  │
│  │     • LoyaltyPenetration           │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  🏷️ Promotions_Pricing.tmdl        │  │         │                                                                  │
│  │     • PromoROI, PromoLift          │  │         │                                                                  │
│  │     • DiscountDepth                │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  🏪 Store_Operations.tmdl          │  │         │                                                                  │
│  │     • Footfall, Conversion         │  │         │                                                                  │
│  │     • WasteRate, LabourCost        │  │         │                                                                  │
│  └─────────────────┬──────────────────┘  │         │                                                                  │
│                    │                     │         │                                                                  │
│                    ▼                     │         │                                                                  │
│  ┌────────────────────────────────────┐  │         │                                                                  │
│  │     🤖 MS Fabric Agent             │  │         │                                                                  │
│  │  "Query needs Snowflake data for   │  │         │                                                                  │
│  │   warehouse-level analysis..."     │──┼─────────┼──────────┐                                                       │
│  └────────────────────────────────────┘  │         │          │                                                       │
│                                          │         │          ▼                                                       │
│                                          │         │  ┌───────────────┐                                               │
│                                          │         │  │ 🔒 AGENT      │                                               │
│                                          │         │  │   GATEWAY     │                                               │
│                                          │         │  │ (Auth/Audit)  │                                               │
│                                          │         │  └───────┬───────┘                                               │
│                                          │         │          │                                                       │
│                                          │         │          ▼                                                       │
│                                          │         │  ┌────────────────────────────────────────────────────────────┐  │
│                                          │         │  │              🔀 DOMAIN ROUTER (Cortex)                     │  │
│                                          │         │  │   "Margin in Scotland → needs Sales + Promo + Ops"         │  │
│                                          │         │  └──────┬──────────────┬──────────────┬───────────────────────┘  │
│                                          │         │         │              │              │                          │
│                                          │         │         ▼              ▼              ▼                          │
│                                          │         │  ┌────────────┐ ┌────────────┐ ┌────────────┐                    │
│                                          │         │  │ 📊 Sales   │ │ 🏷️ Promo   │ │ 🏪 Ops     │                    │
│                                          │         │  │ Cortex SV  │ │ Cortex SV  │ │ Cortex SV  │ ← YAML format      │
│                                          │         │  │ (YAML)     │ │ (YAML)     │ │ (YAML)     │                    │
│                                          │         │  └─────┬──────┘ └─────┬──────┘ └─────┬──────┘                    │
│                                          │         │        │              │              │                          │
│                                          │         │        ▼              ▼              ▼                          │
│                                          │         │  ┌────────────┐ ┌────────────┐ ┌────────────┐                    │
│                                          │         │  │ 🤖 Sales   │ │ 🤖 Promo   │ │ 🤖 Ops     │                    │
│                                          │         │  │   Agent    │ │   Agent    │ │   Agent    │                    │
│                                          │         │  │            │ │            │ │            │                    │
│                                          │         │  │ "Margin    │ │ "PromoMix  │ │ "Waste up  │                    │
│                                          │         │  │  23.4% vs  │ │  +12% vs   │ │  +0.4% in  │                    │
│                                          │         │  │  25% tgt"  │ │  plan"     │ │  fresh"    │                    │
│                                          │         │  └─────┬──────┘ └─────┬──────┘ └─────┬──────┘                    │
│                                          │         │        │              │              │                          │
│                                          │         │        └──────────────┼──────────────┘                          │
│                                          │         │                       ▼                                         │
│                                          │         │  ┌────────────────────────────────────────────────────────────┐  │
│                                          │         │  │                👔 SUPERVISOR AGENT                         │  │
│                                          │         │  │                                                            │  │
│                                          │         │  │   SYNTHESIZES:                                             │  │
│                                          │         │  │   "Margin dropped in Scotland due to:                      │  │
│                                          │         │  │    1. Heavy promotional activity (PromoMix +12%)           │  │
│                                          │         │  │    2. Fresh category waste increased (+0.4%)               │  │
│                                          │         │  │    3. Labour overtime during refit period (+£45k)"         │  │
│                                          │         │  │                                                            │  │
│  ┌────────────────────────────────────┐  │◄────────┼──┴────────────────────────────────────────────────────────────┘  │
│  │     📊 Power BI Renderer           │  │         │                                                                  │
│  │                                    │  │         │                                                                  │
│  │  Generates visualization:          │  │         │                                                                  │
│  │  • Margin waterfall chart          │  │         │                                                                  │
│  │  • Regional heatmap                │  │         │                                                                  │
│  │  • Driver breakdown table          │  │         │                                                                  │
│  └─────────────────┬──────────────────┘  │         │                                                                  │
│                    │                     │         │                                                                  │
│                    ▼                     │         │                                                                  │
│  ┌────────────────────────────────────┐  │         │                                                                  │
│  │  💬 Response to User               │  │         │                                                                  │
│  │                                    │  │         │                                                                  │
│  │  "Margin was 23.4% vs 25% target.  │  │         │                                                                  │
│  │   Key drivers:                     │  │         │                                                                  │
│  │   • Promo depth increased 12%      │  │         │                                                                  │
│  │   • Fresh waste up in 8 stores     │  │         │                                                                  │
│  │   • Labour overtime during refit   │  │         │                                                                  │
│  │                                    │  │         │                                                                  │
│  │   [📊 View Margin Analysis]        │  │         │                                                                  │
│  │   [📈 Open in Power BI]"           │  │         │                                                                  │
│  └────────────────────────────────────┘  │         │                                                                  │
│                                          │         │                                                                  │
└──────────────────────────────────────────┘         └──────────────────────────────────────────────────────────────────┘
```

## Key Concepts

### 1. Dual Semantic Layer

| Aspect | Power BI (TMDL) | Snowflake (Cortex YAML) |
|--------|-----------------|-------------------------|
| **Format** | TMDL / DAX | YAML |
| **Measures** | DAX expressions | SQL expressions |
| **Location** | Microsoft Fabric | Snowflake Stage |
| **Runtime** | Power BI Engine | Cortex Analyst |
| **Visualization** | Native PBI visuals | Returns to PBI |

### 2. Agent Roles

| Agent | Platform | Responsibility |
|-------|----------|----------------|
| **PBI Copilot** | Microsoft | Entry point, simple queries |
| **MS Fabric Agent** | Microsoft | Orchestrates PBI side, initiates handoff |
| **Agent Gateway** | Shared | Auth, audit, governance |
| **Domain Router** | Snowflake | Routes to relevant domains |
| **Domain Agents** (5) | Snowflake | Query specific semantic views |
| **Supervisor** | Snowflake | Synthesize multi-domain findings |
| **PBI Renderer** | Microsoft | Generate visualizations |

### 3. When Does Handoff Happen?

**Stay in Power BI** (no handoff):
- Simple metric lookups ("What was last week's sales?")
- Pre-built dashboard navigation
- Single-domain, single-measure queries

**Handoff to Snowflake** when:
- Cross-domain analysis needed ("Why did X affect Y?")
- Real-time data required (PBI has stale snapshot)
- Complex multi-step reasoning
- Access to data not replicated to PBI

### 4. Data Flow Summary

```
1. USER → PBI Copilot: "Why did margin drop in Scotland?"
   
2. PBI Copilot → Checks TMDL models
   ↳ Basic answer available? → Respond directly
   ↳ Needs deeper analysis? → Continue to step 3

3. MS Fabric Agent → Agent Gateway → Snowflake Router
   ↳ Packages: query, context, user permissions
   ↳ Gateway validates auth, logs request

4. Domain Router → Identifies: Sales + Promo + Ops needed
   ↳ Parallel dispatch to 3 domain agents

5. Domain Agents → Query Cortex Semantic Views
   ↳ Sales Agent: "Margin 23.4% vs 25%"
   ↳ Promo Agent: "PromoMix +12%"
   ↳ Ops Agent: "Waste +0.4%, OT +£45k"

6. Supervisor → Synthesizes findings
   ↳ "Margin down due to: promo activity, waste, overtime"

7. Supervisor → PBI Renderer
   ↳ Data formatted for visualization

8. PBI Renderer → Generates chart
   ↳ Margin waterfall, regional heatmap

9. Response → User
   ↳ Text insight + embedded visualization
```

## Why This Architecture?

1. **Best of Both Worlds**
   - Power BI's rich visualization + Snowflake's compute power
   - TMDL for business-friendly measures + YAML for enterprise governance

2. **Separation of Concerns**
   - Microsoft owns visualization layer
   - Snowflake owns data processing layer
   - Clear handoff points with audit trail

3. **Domain Expertise**
   - Each domain agent is specialized (Sales knows margin, Ops knows waste)
   - Supervisor connects the dots across domains

4. **Governance**
   - Agent Gateway enforces policies
   - Both platforms maintain their own semantic models
   - Audit trail of all cross-platform queries



## Visual Architecture

```
┌──────────────────────────────────────────┐         ┌──────────────────────────────────────────────────────────────────┐
│           🟦 MICROSOFT SIDE              │         │                    ❄️ SNOWFLAKE SIDE                             │
│          (Power BI / Fabric)             │         │                   (Cortex / Warehouse)                          │
├──────────────────────────────────────────┤         ├──────────────────────────────────────────────────────────────────┤
│                                          │         │                                                                  │
│  ┌────────────────────────────────────┐  │         │                                                                  │
│  │  👤 User asks in Power BI Copilot  │  │         │                                                                  │
│  │  "Why did margin drop in Scotland?"│  │         │                                                                  │
│  └─────────────────┬──────────────────┘  │         │                                                                  │
│                    │                     │         │                                                                  │
│                    ▼                     │         │                                                                  │
│  ┌────────────────────────────────────┐  │         │                                                                  │
│  │      📊 TMDL Semantic Models       │  │         │                                                                  │
│  │      (DAX measures, tables)        │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  📈 Sales_Revenue.tmdl             │  │         │                                                                  │
│  │     • TotalSales, LFLSales         │  │         │                                                                  │
│  │     • GrossMarginPct, ATV          │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  📦 Inventory_Supply.tmdl          │  │         │                                                                  │
│  │     • StockTurn, InStockRate       │  │         │                                                                  │
│  │     • DaysOfSupply, OTIF           │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  👥 Customer_Loyalty.tmdl          │  │         │                                                                  │
│  │     • CLV, RetentionRate           │  │         │                                                                  │
│  │     • LoyaltyPenetration           │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  🏷️ Promotions_Pricing.tmdl        │  │         │                                                                  │
│  │     • PromoROI, PromoLift          │  │         │                                                                  │
│  │     • DiscountDepth                │  │         │                                                                  │
│  ├────────────────────────────────────┤  │         │                                                                  │
│  │  🏪 Store_Operations.tmdl          │  │         │                                                                  │
│  │     • Footfall, Conversion         │  │         │                                                                  │
│  │     • WasteRate, LabourCost        │  │         │                                                                  │
│  └─────────────────┬──────────────────┘  │         │                                                                  │
│                    │                     │         │                                                                  │
│                    ▼                     │         │                                                                  │
│  ┌────────────────────────────────────┐  │         │                                                                  │
│  │     🤖 MS Fabric Agent             │  │         │                                                                  │
│  │  "Query needs Snowflake data for   │  │         │                                                                  │
│  │   warehouse-level analysis..."     │──┼─────────┼──────────┐                                                       │
│  └────────────────────────────────────┘  │         │          │                                                       │
│                                          │         │          ▼                                                       │
│                                          │         │  ┌───────────────┐                                               │
│                                          │         │  │ 🔒 AGENT      │                                               │
│                                          │         │  │   GATEWAY     │                                               │
│                                          │         │  │ (Auth/Audit)  │                                               │
│                                          │         │  └───────┬───────┘                                               │
│                                          │         │          │                                                       │
│                                          │         │          ▼                                                       │
│                                          │         │  ┌────────────────────────────────────────────────────────────┐  │
│                                          │         │  │              🔀 DOMAIN ROUTER (Cortex)                     │  │
│                                          │         │  │   "Margin in Scotland → needs Sales + Promo + Ops"         │  │
│                                          │         │  └──────┬──────────────┬──────────────┬───────────────────────┘  │
│                                          │         │         │              │              │                          │
│                                          │         │         ▼              ▼              ▼                          │
│                                          │         │  ┌────────────┐ ┌────────────┐ ┌────────────┐                    │
│                                          │         │  │ 📊 Sales   │ │ 🏷️ Promo   │ │ 🏪 Ops     │                    │
│                                          │         │  │ Cortex SV  │ │ Cortex SV  │ │ Cortex SV  │ ← YAML format      │
│                                          │         │  │ (YAML)     │ │ (YAML)     │ │ (YAML)     │                    │
│                                          │         │  └─────┬──────┘ └─────┬──────┘ └─────┬──────┘                    │
│                                          │         │        │              │              │                          │
│                                          │         │        ▼              ▼              ▼                          │
│                                          │         │  ┌────────────┐ ┌────────────┐ ┌────────────┐                    │
│                                          │         │  │ 🤖 Sales   │ │ 🤖 Promo   │ │ 🤖 Ops     │                    │
│                                          │         │  │   Agent    │ │   Agent    │ │   Agent    │                    │
│                                          │         │  │            │ │            │ │            │                    │
│                                          │         │  │ "Margin    │ │ "PromoMix  │ │ "Waste up  │                    │
│                                          │         │  │  23.4% vs  │ │  +12% vs   │ │  +0.4% in  │                    │
│                                          │         │  │  25% tgt"  │ │  plan"     │ │  fresh"    │                    │
│                                          │         │  └─────┬──────┘ └─────┬──────┘ └─────┬──────┘                    │
│                                          │         │        │              │              │                          │
│                                          │         │        └──────────────┼──────────────┘                          │
│                                          │         │                       ▼                                         │
│                                          │         │  ┌────────────────────────────────────────────────────────────┐  │
│                                          │         │  │                👔 SUPERVISOR AGENT                         │  │
│                                          │         │  │                                                            │  │
│                                          │         │  │   SYNTHESIZES:                                             │  │
│                                          │         │  │   "Margin dropped in Scotland due to:                      │  │
│                                          │         │  │    1. Heavy promotional activity (PromoMix +12%)           │  │
│                                          │         │  │    2. Fresh category waste increased (+0.4%)               │  │
│                                          │         │  │    3. Labour overtime during refit period (+£45k)"         │  │
│                                          │         │  │                                                            │  │
│  ┌────────────────────────────────────┐  │◄────────┼──┴────────────────────────────────────────────────────────────┘  │
│  │     📊 Power BI Renderer           │  │         │                                                                  │
│  │                                    │  │         │                                                                  │
│  │  Generates visualization:          │  │         │                                                                  │
│  │  • Margin waterfall chart          │  │         │                                                                  │
│  │  • Regional heatmap                │  │         │                                                                  │
│  │  • Driver breakdown table          │  │         │                                                                  │
│  └─────────────────┬──────────────────┘  │         │                                                                  │
│                    │                     │         │                                                                  │
│                    ▼                     │         │                                                                  │
│  ┌────────────────────────────────────┐  │         │                                                                  │
│  │  💬 Response to User               │  │         │                                                                  │
│  │                                    │  │         │                                                                  │
│  │  "Margin was 23.4% vs 25% target.  │  │         │                                                                  │
│  │   Key drivers:                     │  │         │                                                                  │
│  │   • Promo depth increased 12%      │  │         │                                                                  │
│  │   • Fresh waste up in 8 stores     │  │         │                                                                  │
│  │   • Labour overtime during refit   │  │         │                                                                  │
│  │                                    │  │         │                                                                  │
│  │   [📊 View Margin Analysis]        │  │         │                                                                  │
│  │   [📈 Open in Power BI]"           │  │         │                                                                  │
│  └────────────────────────────────────┘  │         │                                                                  │
│                                          │         │                                                                  │
└──────────────────────────────────────────┘         └──────────────────────────────────────────────────────────────────┘
```

## Key Concepts

### 1. Dual Semantic Layer

| Aspect | Power BI (TMDL) | Snowflake (Cortex YAML) |
|--------|-----------------|-------------------------|
| **Format** | TMDL / DAX | YAML |
| **Measures** | DAX expressions | SQL expressions |
| **Location** | Microsoft Fabric | Snowflake Stage |
| **Runtime** | Power BI Engine | Cortex Analyst |
| **Visualization** | Native PBI visuals | Returns to PBI |

### 2. Agent Roles

| Agent | Platform | Responsibility |
|-------|----------|----------------|
| **PBI Copilot** | Microsoft | Entry point, simple queries |
| **MS Fabric Agent** | Microsoft | Orchestrates PBI side, initiates handoff |
| **Agent Gateway** | Shared | Auth, audit, governance |
| **Domain Router** | Snowflake | Routes to relevant domains |
| **Domain Agents** (5) | Snowflake | Query specific semantic views |
| **Supervisor** | Snowflake | Synthesize multi-domain findings |
| **PBI Renderer** | Microsoft | Generate visualizations |

### 3. When Does Handoff Happen?

**Stay in Power BI** (no handoff):
- Simple metric lookups ("What was last week's sales?")
- Pre-built dashboard navigation
- Single-domain, single-measure queries

**Handoff to Snowflake** when:
- Cross-domain analysis needed ("Why did X affect Y?")
- Real-time data required (PBI has stale snapshot)
- Complex multi-step reasoning
- Access to data not replicated to PBI

### 4. Data Flow Summary

```
1. USER → PBI Copilot: "Why did margin drop in Scotland?"
   
2. PBI Copilot → Checks TMDL models
   ↳ Basic answer available? → Respond directly
   ↳ Needs deeper analysis? → Continue to step 3

3. MS Fabric Agent → Agent Gateway → Snowflake Router
   ↳ Packages: query, context, user permissions
   ↳ Gateway validates auth, logs request

4. Domain Router → Identifies: Sales + Promo + Ops needed
   ↳ Parallel dispatch to 3 domain agents

5. Domain Agents → Query Cortex Semantic Views
   ↳ Sales Agent: "Margin 23.4% vs 25%"
   ↳ Promo Agent: "PromoMix +12%"
   ↳ Ops Agent: "Waste +0.4%, OT +£45k"

6. Supervisor → Synthesizes findings
   ↳ "Margin down due to: promo activity, waste, overtime"

7. Supervisor → PBI Renderer
   ↳ Data formatted for visualization

8. PBI Renderer → Generates chart
   ↳ Margin waterfall, regional heatmap

9. Response → User
   ↳ Text insight + embedded visualization
```

## Why This Architecture?

1. **Best of Both Worlds**
   - Power BI's rich visualization + Snowflake's compute power
   - TMDL for business-friendly measures + YAML for enterprise governance

2. **Separation of Concerns**
   - Microsoft owns visualization layer
   - Snowflake owns data processing layer
   - Clear handoff points with audit trail

3. **Domain Expertise**
   - Each domain agent is specialized (Sales knows margin, Ops knows waste)
   - Supervisor connects the dots across domains

4. **Governance**
   - Agent Gateway enforces policies
   - Both platforms maintain their own semantic models
   - Audit trail of all cross-platform queries





